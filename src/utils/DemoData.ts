...
export const generateCurrentDayDemo = () => {
  ...
  if (currentHour >= 9 && currentHour < 15) {
    ...
    if (currentClass) {
      currentClass.startTime = currentTimeStr;
      currentClass.endTime = `${currentHour.toString().padStart(2, '0')}:45`;
    }
    
    // Make next class
    const nextClass = schedule[1];
    if (nextClass) {
      nextClass.startTime = nextTimeStr;
      nextClass.endTime = `${nextHour.toString().padStart(2, '0')}:45`;
    }
  }
